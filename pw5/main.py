...
def save_to_txt(classroom: Classroom, courses: Courses):
    ...
    with open("marks.txt", "w") as f:
        for mark in marks:
            f.write(f"{','.join([str(i) for i in mark])}\n")


def read_from_txt(classroom: Classroom, courses: Courses):
    with open("students.txt", "r") as f:
        for line in f.readlines():
            student_id, student_name, _student_dob = line.strip().split(",")
            student_dob = datetime.strptime(_student_dob, "%Y-%m-%d")
            classroom.students.append(Student(student_id, student_name, student_dob))

    with open("courses.txt", "r") as f:
        for line in f.readlines():
            course_id, course_name = line.strip().split(",")
            courses.courses.append(BasicInfo(course_id, course_name))

    with open("marks.txt", "r") as f:
        for line in f.readlines():
            student_id, course_id, attendance, midterm, final = line.strip().split(",")
            for student in classroom.students:
                if student.id == student_id:
                    for course_marks in student.courses_marks:
                        if course_marks.course_id == course_id:
                            course_marks.attendance = float(attendance)
                            course_marks.midterm = float(midterm)
                            course_marks.final = float(final)
                            break
                    else:
                        course_marks = Mark(course_id, float(attendance), float(midterm), float(final))
